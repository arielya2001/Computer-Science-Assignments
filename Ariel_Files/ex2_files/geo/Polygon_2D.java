...
public class Polygon_2D implements GeoShape {
	...
	/** equals
	 * Checks if the current polygon is equal to the specified object.
	 * Two polygons are considered equal if they have the same number of points,
	 * and if each corresponding pair of points is equal.
	 * @param obj The object to compare with this polygon.
	...
	public boolean equals(Object obj) {
		...
		Polygon_2D otherPolygon = (Polygon_2D) obj;

		Point_2D[] thisPoints = this.getAllPoints();
		Point_2D[] otherPoints = otherPolygon.getAllPoints();

		if (thisPoints.length != otherPoints.length) return false;

		for (int i = 0; i < thisPoints.length; i++) {
			if (!thisPoints[i].equals(otherPoints[i])) return false;
		}
