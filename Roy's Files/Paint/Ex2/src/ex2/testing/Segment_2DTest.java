...
class Segment_2DTest {
    ...
    static Stream<Arguments> segmentArguments() {
        return Stream.of(
                Arguments.of(
                        new Segment_2D(new Point_2D(0, 0), new Point_2D(1, 1)),
                        new Segment_2D(new Point_2D(0, 0), new Point_2D(1, 1)),
                        true
                ),
                Arguments.of(
                        new Segment_2D(new Point_2D(0, 0), new Point_2D(1, 0)),
                        new Segment_2D(new Point_2D(0, 0), new Point_2D(2, 0)),
                        false
                ),
                Arguments.of(
                        new Segment_2D(new Point_2D(0, 0), new Point_2D(0, 1)),
                        new Segment_2D(new Point_2D(0, 1), new Point_2D(0, 0)),
                        true  // Segments are equal if they have the same endpoints, regardless of order
                ),
                Arguments.of(
                        new Segment_2D(new Point_2D(1, 1), new Point_2D(1, 3)),
                        new Segment_2D(new Point_2D(1, 1), new Point_2D(1, 3)),
                        true
                ));
    }
    ...
    @ParameterizedTest
    @MethodSource("segmentArguments")
    public void Equals(Segment_2D segment1, Segment_2D segment2, boolean expected) {
        assertEquals(expected, segment1.equals(segment2));
    }
